<template>
  <div class="analog-outputs">
    ...
    <div class="page-header">
      <h1>Analog Output Objects</h1>
      <p>Manage analog output objects in the BACnet system</p>
    </div>
...
const dialogTitle = computed(() => {
  return isEdit.value ? 'Edit Analog Output' : 'Create Analog Output'
})
...
async function loadData() {
  loading.value = true
  try {
    const response = await apiService.getAnalogOutputs({
    ...
    console.error('Failed to load analog outputs:', error)
    ElMessage.error('Failed to load analog outputs')
  } finally {
    loading.value = false
  }
}
...
async function deleteObject(row) {
  try {
    await ElMessageBox.confirm(
      `Are you sure you want to delete analog output "${row.object_name}"?`,
      'Confirm Delete',
      {
    ...
    await apiService.deleteAnalogOutput(row.id)
    ElMessage.success('Analog output deleted successfully')
    loadData()
  } catch (error) {
    if (error !== 'cancel') {
      console.error('Failed to delete analog output:', error)
      ElMessage.error('Failed to delete analog output')
    }
  }
}
...
async function submitForm() {
  ...
  try {
    ...
    if (isEdit.value) {
      await apiService.updateAnalogOutput(formData.id, formData)
      ElMessage.success('Analog output updated successfully')
    } else {
      await apiService.createAnalogOutput(formData)
      ElMessage.success('Analog output created successfully')
    }
    
    dialogVisible.value = false
    loadData()
  } catch (error) {
    console.error('Failed to save analog output:', error)
    ElMessage.error('Failed to save analog output')
  } finally {
